"""Native cognis-connect emit for accessreview — forward findings to any platform.

Maps accessreview's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    accessreview ... --format json | accessreview-emit --to stix
    accessreview-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    """Tailor a accessreview record toward the Finding contract before normalize().
